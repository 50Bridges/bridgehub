...
async function loadAndRender() {
  ...
  if (idx.guideline >= 0) {
    displayCols.push({ idx: idx.guideline, type: 'guideline', label: 'Guideline' });
  }
  ...
  function renderTable() {
    ...
    let rows = data.filter(r => {
      ...
      if (companyQuery.length >= 3) {
        if (!r[idx.name].toLowerCase().startsWith(companyQuery)) return false;
      }

      if (wholesalerQuery.length >= 3) {
        if (!r[idx.wholesaler].toLowerCase().startsWith(wholesalerQuery)) return false;
      }
    ...
    const sortIdx = headerRaw.findIndex(h => h.toLowerCase() === currentSort.by);
    ...
    displayCols.forEach(col => {
      ...
      if (col.idx === idx.wholesaler || col.idx === idx.name) {
        ...
        btn.textContent =
          currentSort.by === headerKey[col.idx]
        ...
        btn.addEventListener('click', () => {
          currentSort = {
            ...
            dir:
              currentSort.by === headerKey[col.idx] && currentSort.dir === 'asc'
                ? 'desc'
                : 'asc'
          };
          renderTable();
    ...
    rows.forEach(r => {
      ...
      checkbox.addEventListener('change', e => {
        e.target.checked ? selectedRows.add(rowId) : selectedRows.delete(rowId);
        updateActionButtons();
      });
